...
# -*- coding: utf8 -*-
"""
Creates a json file with information about all datasheet files in given directory path.
The output file, named '{}' by default, is also stored into given directory path.
...
if __name__ == '__main__':
    ...
    args_parser = argparse. ArgumentParser(
        ...
        description=textwrap.dedent(__doc__.format(DEFAULT_FILE_NAME,
                                                   script_name,
                                                   script_name)))
    args_parser.add_argument('ds_files', metavar='path/to/directory', nargs=1,
                             help='Directory where files are stored')
    args_parser.add_argument('-o', '--out-file', metavar='file-name', nargs='?',
                             required=False, default=DEFAULT_FILE_NAME,
                             help='File where result is stored to (default: "{}")'.format(DEFAULT_FILE_NAME))
    ...
    if not ds_file.exists():
        args_parser.error('Given path "{}" does not exists.'.format(args.ds_files[0]))

    # given path must exist and a folder
    if not ds_file.is_dir():
        args_parser.error('Given path "{}" is not a folder!'.format(args.ds_files[0]))

    _working_directory = ds_file.absolute()
    # outfile is stored in given directory; check if suffix json is present, add if necessary
    out_file = pathlib.Path(_working_directory, args.out_file).with_suffix('.json')
