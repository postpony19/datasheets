            ...
            <xsl:for-each select="bahnhof">
               <table border="1">
                  <tr>
                     ...
                     <td class="links" colspan="2"><xsl:text disable-output-escaping="yes">Kürzel: </xsl:text><span><xsl:value-of select="kuerzel"/></span></td>
                  </tr>
                  <tr>
                     <td class="links" colspan="2">Typ: <span><xsl:value-of select="typ"/></span></td>
							<td class="links" colspan="4"><xsl:text disable-output-escaping="yes">Maßstab: </xsl:text><span>1:<xsl:value-of select="maszstab"/></span></td>
                     <td class="links" colspan="4">Modulnr.: <span><xsl:value-of select="modulnr"/></span></td>
                  </tr>
   ...
   <xsl:template match="gv">
      <tr>
         <td>Ladestelle</td>
			<td class="mitte">Gleis</td>
         <td class="rechts"><xsl:text disable-output-escaping="yes">Länge</xsl:text></td>
			<td class="rechts"><xsl:text disable-output-escaping="yes">Länge (Achsen)</xsl:text></td>
      ...
      <xsl:apply-templates select="ladestelle"/>
		<tr style="font-weight:bold">
			<td colspan="{$maxcols -7}" class="rechts">Summe:</td>
			<td class="rechts">
			<xsl:call-template name="produkt_summe">
      ...
      <xsl:call-template name="leerzeile"/>
      <tr>
         <td colspan="{$maxcols -7}"><xsl:text disable-output-escaping="yes">Versender/Empfänger</xsl:text></td>
			<td class="mitte">Versand/Empfang</td>
			<td colspan="{$maxcols -7}">Ladegut</td>
         <td class="mitte">Wagengattung</td>
			<td class="mitte">Ladestelle</td>
			<td>Wagenmenge</td>
      </tr>
   ...
   <xsl:template match="wagen">
      ...
      <xsl:choose>
         <xsl:when test="@zeitraum">
            <xsl:choose>
               <xsl:when test="@zeitraum='tag'">Tag</xsl:when>
					<xsl:when test="@zeitraum='woche'">Woche</xsl:when>
				</xsl:choose>
			</xsl:when>
