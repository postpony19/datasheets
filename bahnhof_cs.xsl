...
<xsl:output method="html" encoding="utf-8"/>
   ...
   <xsl:template match="bemerkung">
	...
	<xsl:template name="caroutput">
		...
		<tr>
			<td colspan="{$maxcols -7}" style="text-align:right;"><xsl:text>příjem</xsl:text></td>
		...
		<tr>
			<td colspan="{$maxcols -7}" style="text-align:right;"><xsl:text>odeslání/příjem</xsl:text></td>
		...
		<tr style="font-weight:bold;">
			<td colspan="{$maxcols -7}" style="text-align:right;"><xsl:text>celkem</xsl:text></td>
		...
		<tr>
		   <td colspan="{$maxcols}"><xsl:text disable-output-escaping="yes">Poznámka: Výsledky jsou vypočteny za předpokladu 7-denního týdne a bez opětovného nakládání prázdných vozů!</xsl:text></td>
		</tr>
	</xsl:template>
</xsl:stylesheet>
