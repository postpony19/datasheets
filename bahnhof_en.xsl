...
<xsl:output method="html" encoding="utf-8"/>
            ...
            <xsl:for-each select="bahnhof">
               <table border="1">
                  ...
                  <tr>
                     ...
                     <td class="links" colspan="4">Module ID: <span><xsl:value-of select="modulnr"/></span></td>
                  </tr>
   ...
   <xsl:template match="gleise">
      <tr>
         ...
         <td class="rechts"><xsl:text disable-output-escaping="yes">Effective length</xsl:text></td>
         <td class="rechts"><xsl:text disable-output-escaping="yes">Effective length (axles)</xsl:text></td>
         <td colspan="{$maxcols -4}"><xsl:text disable-output-escaping="yes">&#160;Remarks</xsl:text></td>
      </tr>
		<tr><td colspan="{$maxcols}"><span class="m">Main tracks</span></td></tr>
		...
		<xsl:if test="$content &gt; 0">
			<tr><td colspan="{$maxcols}"><span class="m">Side tracks</span></td></tr>
   ...
   <xsl:template match="pv">
      <tr>
         ...
         <td class="mitte">Track</td>
			...
			<td colspan="{$maxcols -3}" rowspan="{count(bahnsteig)+1}" class="mitte" valign="top">Remarks to passenger traffic:<br/>
				<span class="m">
						<xsl:call-template name="recurse_break">
   ...
   <xsl:template match="gv">
      <tr>
         <td>Loading place</td>
			<td class="mitte">Track</td>
         <td class="rechts"><xsl:text disable-output-escaping="yes">Length</xsl:text></td>
			<td class="rechts"><xsl:text disable-output-escaping="yes">Length (axles)</xsl:text></td>
			<td colspan="{$maxcols -4}" rowspan="{count(ladestelle)+1}" class="mitte" valign="top">
			<xsl:text disable-output-escaping="yes">Remarks to cargo traffic:</xsl:text>
			<br/><span class="m">
	      <xsl:call-template name="recurse_break">
				<xsl:with-param name="idlist" select="bemerkung"/>
	      </xsl:call-template>
      ...
      <xsl:call-template name="leerzeile"/>
      <tr>
         <td colspan="{$maxcols -7}"><xsl:text disable-output-escaping="yes">Distributor/Receiver</xsl:text></td>
			<td class="mitte">Distribution/Receiving</td>
			<td colspan="{$maxcols -7}">Goods</td>
         <td class="mitte">Car type</td>
			<td class="mitte">Loading place</td>
			<td>Car quantity</td>
      </tr>
   ...
   <xsl:template match="bemerkung">
      <tr>
         <td colspan="{$maxcols}">General remarks to station:<br/><br/>
				<span class="m">
				<!--<xsl:value-of select="."/>-->
						<xsl:call-template name="recurse_break">
